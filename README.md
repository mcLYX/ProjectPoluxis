...
- 空白处拖拽：仅滚动播放头，不会放置音符。
- 放置音符后按住不放：上下拖动可调整该音符的 Y 值（需拖出一定距离，步进 0.1；向上为增大，向下为减小）。

---

## 多选与批量编辑

「选择 / 移动」工具支持一次选中多个音符，进行批量移动与批量编辑。

### 进入多选模式

- **双击「选择 / 移动」工具图标**：选中框由青色变为金色，文字变为「多选 / 移动」；再次双击切回单选。
- **双击 R 键**：在多选 / 单选模式之间切换。
- **按住 Ctrl（电脑端）**：任意工具下临时进入多选，松手恢复原工具；若已在多选模式，按住 Ctrl 则临时回到单选。

### 多选模式下的操作

- **单击音符**：选中；再次单击同一音符：取消选中。可以依次点选多个音符。
- **2D 视图框选**：在音符画布内按住并拖动，会拖出一个选框，框住的所有音符（含链式音符的子节点）被选中。在画布外的空白处拖动仍为滚动播放头。
  - 框选方式可在「批量编辑」弹窗中调整：**正常**（重新选中框内音符）、**叠加**（把框内音符加入已有选中）、**减去**（从已选中中移除框内音符）、**交叉**（框内音符在选中 / 未选中间翻转）。
- **批量移动**：拖拽任意一个已选中的音符，所有选中音符会同步同幅度移动（X 限制 ±2.4、Y 限制 ±1.5）。
- **子节点可独立选中**：链式音符（Slide / Touch）的子节点被视为独立音符，点击只选中该子节点，拖动也只会移动它，而不会带动整条链。

### 批量编辑弹窗

选中音符后，原「音符链编辑」弹窗会变为「**批量编辑**」弹窗（多选模式下选中 1 个也会显示）。其中提供：

| 按钮 / 功能 | 作用 |
| --- | --- |
| **克隆** | 把选中的音符复制一份到当前播放头位置（以最早选中的音符为 0 时刻），原选中保持 |
| **删除** | 删除选中的音符（需二次确认） |
| **功能下拉 + 执行** | 执行下方某一项批量功能 |

**批量功能**（在功能下拉中选择，点击「执行」生效）：

| 功能 | 作用 |
| --- | --- |
| ToTap / ToTouch / ToSlide | 把选中的音符转换为指定类型。只改动选中的节点：链中未被选中的部分会按连续段自动重连成新链，被断开处产生的新链头继承原链头的外观参数 |
| MkChain（连成链） | 把选中的音符按时间顺序桥接成一条链；被绕过的节点会断开成独立音符 |
| SplitChain（拆散链） | 断开选中相邻节点之间的连接。需选中至少两个相连的相邻节点才生效；整条链全部选中则全部拆散为独立音符 |
| Fill（线性 / Sine-in / Sine-out / Sine-in&out） | 在相邻选中音符之间，按当前吸附间隔插入过渡音符；坐标按所选缓动方式插值 |
| FlipX / FlipY | 把选中音符的 X / Y 坐标取反（只影响选中的节点） |
| UseRule | 对每个选中音符应用「放置规则」面板中的脚本 |

### 区间操作

原「批量选择与操作」面板更名为「**区间操作**」，除原有的区间批量操作外，新增两个按钮：

- **全部选中**：把区间内所有音符一次性选中。
- **应用放置规则**：对区间内所有音符应用「放置规则」脚本（子节点同样生效）。
